...
% S(:,[1 2 4 6 7])=detrend(log(S(:,[1 2 4 6 7]))); % log-linearly detrended, L
    ...
    % CONT HERE: figure out also what default_index and Y_def really are??
...
Sd1 = Sd; % size changes with each time I run the loop, so it must depend on default
Sd=permute(reshape(Sd,2*debtmax+1,size(Sd,1)/(2*debtmax+1),num_var+2),[1 3 2]); % reshape this matrix so that it now is (number of debt levels) x (number of variables) x (number of defaults per each debt level) 
Sd2 = Sd;
Sd(:,5,:)=bsxfun(@rdivide,Sd(:,5,:),Sd(1,5,:)); % WTF is this?
Sd3 = Sd;
Sdm=nanmean(Sd,3); %take means along 3rd dimension, i.e.  we get means for all debt levels x for all variables [all of this for default values, recall]
